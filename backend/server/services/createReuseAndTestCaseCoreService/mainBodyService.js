module.exports = {
    /*
    Logic Desc: this function is mainly to build the logic of scritp body.
    here the call divide on seperate function based on the group, each group had 
    its own responsibiltes to generate the code.
    here promise are used for better performances.
    query: no.
    Result: Each fucntion resloves an object called actionForObjectScript, that will catched by its next function.
     */
    url1(data, templatePath, scriptPath, callback) {
    ...
    Reporter.log(actions.CaptureScreenShotAtEachStep(path,methodName,screenshotOption));
        ...
        actions.AssertAll(); 
        driver.get().quit();`;
        const getMethod = `String varMethodName=Thread.currentThread().getStackTrace()[1].getMethodName();`
        const catchBlock = `}
        catch(Exception e)
        {
        String methodName=Thread.currentThread().getStackTrace()[1].getMethodName();     
        ...
        lr.on('line', function (line) {
            if (line.includes(fName)) {
                ...
                if (data.typeOfFunction == 'reusableFunction') {
                    NewLine = oldLine.replace(changeString, data.fileName)
                }
                else {
                    var NewLine = oldLine.replace(changeString, data.scriptId)
                }
                ...
                data.allObjectData.allActitons.forEach(scriptValue => {
                    ...
                    else if (scriptValue.Groups === "Scroll") {
                        scrollMethodHandling(scriptValue).then((resp) => { makeCommonCallForPush(resp, data) })
                    }
                    else {
                        // ifFunction();
                    }
        ...
        var pomObjectNotFunction = (scriptValue) => new Promise((resolves, rejects) => {
            ...
            if (scriptValue.Input2 != undefined && scriptValue.Input2 != '' && scriptValue.Input3 != undefined && scriptValue.Input3 != '' && scriptValue.Excel == "notExcel") {
                // var scriptValueGene = `actions.${scriptValue.ActionList}("${scriptValue.Input2}");`;
                console.log('1')
                const replaceIfVariablePresent2 = data.allVariablesForScript.filter(allVariables => allVariables.variableAddFun === scriptValue.Input2);
                const replaceIfVariablePresent3 = data.allVariablesForScript.filter(allVariables => allVariables.variableAddFun === scriptValue.Input3);
                const replaceIfParamPresent = data.reusableVar.filter(reuseVar => reuseVar === scriptValue.Input2);
                if (replaceIfVariablePresent2.length === 1 && replaceIfVariablePresent3 === 1 ) {
                    var scriptValueGene = `actions.${scriptValue.ActionList}(${scriptValue.Input2},${scriptValue.Input3});`;
                }
                else if (replaceIfVariablePresent2.length === 1) {
                    var scriptValueGene = `actions.${scriptValue.ActionList}(${scriptValue.Input2},"${scriptValue.Input3}");`;
                }
                else if (replaceIfVariablePresent3.length === 1) {
                    var scriptValueGene = `actions.${scriptValue.ActionList}("${scriptValue.Input2}",${scriptValue.Input3});`;
                }
                else if (replaceIfParamPresent.length === 1) {
                    var scriptValueGene = `actions.${scriptValue.ActionList}(${scriptValue.Input2},${scriptValue.Input3});`;
                }
                else {
                    var scriptValueGene = `actions.${scriptValue.ActionList}("${scriptValue.Input2}","${scriptValue.Input3}");`;
            ...
            else if (scriptValue.Input2 != undefined && scriptValue.Input2 != '' && scriptValue.Excel == "notExcel") {
                console.log('2')
                // var scriptValueGene = `actions.${scriptValue.ActionList}("${scriptValue.Input2}");`;
                ...
                const replaceIfParamPresent = data.reusableVar.filter(reuseVar => reuseVar === scriptValue.Input2);
                if(scriptValue.ActionList=='PrintVariable'){
                 if (replaceIfVariablePresent.length === 1) {
                        var scriptValueGene = `actions.${scriptValue.ActionList}(${scriptValue.Input2}, "${scriptValue.Input2}", varMethodName);`;
                 }
                    else if (replaceIfParamPresent.length === 1) {
                        var scriptValueGene = `actions.${scriptValue.ActionList}(${scriptValue.Input2}, "${scriptValue.Input2}", varMethodName);`;
                    }
                ...
                else { var scriptValueGene = "actions." + scriptValue.ActionList + "();"; }
            }//forReturnValue
        ...
        */
        ...
        var owsapzapMethodHandlingFunction = (scriptValue) => new Promise((resloves, rejects) => {
            var actionObjForScript = {};
            if (scriptValue.ActionList === 'generateZapReport') {
                var scriptValueGene = "actions." + scriptValue.ActionList + "(zapIPAddress);";
            }
            else{
                var scriptValueGene = "actions." + scriptValue.ActionList + "();";
            }
            ...
            if (methodParams === '' || methodParams === undefined) {
                ...
                for (i = 0; i < methodParams1.length; i++) {
                    if (i != methodParams1.length - 1) {
                        ...
                        if (result.length !== 0) { demo.push(`${methodParams1[i]}`); }
     ...
     */
        ...
        var appiumCall = (scriptValue) => new Promise((resloves, rejects) => {
            console.log("createTestpostAllActionscreateTestpostAllActionscreateTestpostAllActionscreateTestpostAllActions")
            var actionObjForScript = {};
        ...
        /////////////////////// scrollMethodHandling//////////////////////////

        var scrollMethodHandling = (scriptValue) => new Promise((resloves, rejects) => {
   ...
   */
        function writeTestScriptTOFile(formatted, yash) {
            formatted.forEach((i, index, array) => {
                ...
                if (array[index][0].actionObjForScript.includes('actions.OpenApplication')) {
                    // console.log(array[index][0].actionObjForScript+"else if")
                ...
                ` , 'utf8');
                    ...
                    if(array[index][0].actionObjForScript.includes('actions.PrintVariable')){
           ...
           `, 'utf8');
                    ...
                    console.log(array[index][0].actionObjForScript + "if")
                    // fs.appendFileSync(scriptPath,`${init}${index+1})${startBlock}${index+1}${exceptionBlock}
           ...
           `, 'utf8');
                    ...
                    // console.log(array[index][0].actionObjForScript+"else")
                    fs.appendFileSync(scriptPath, `${init}${index + 1})${startBlock}${index + 1}${exceptionBlock}
            ${callLoop(i)}
            ${commonBlock}
            ${catchBlock}
        ...
        function writeReusFunctiontoFile(formatted, yash) {
            ...
            fs.appendFileSync(scriptPath,`${getMethod}`,'utf8');
            //////////////code to generate return statement//////////////
            let flag = false;
            formatted.forEach((e) => {
                e.forEach(ele => {
                    if (ele.actionObjForScript.includes(yash.reusable.returnVariable) && yash.reusable.returnVariable !== '') {
                        flag = true;
                    }
                })
            })
            ...
            //////////////code to generate return statement ends//////////////
                ...
                if (scriptValue.ReturnsValue != undefined && scriptValue.ReturnsValue != '') {
                    ...
                    if (replace.length === 1) {
                        ...
                        if (scriptValue.ActionList.includes("ByIndex") === true) {
                            ...
                            if (scriptValue.Groups !== `Waits`) {
                                var scriptValueGene = scriptValue.ReturnsValue + `=actions.${scriptValue.ActionList}(${scriptValue.PomObject},"${scriptValue.Input2}");`;
                            }
                            else {
                                if (scriptValue.Input3 === undefined) {
                                    var scriptValueGene = scriptValue.ReturnsValue + `=actions.${scriptValue.ActionList}(${scriptValue.PomObject},${scriptValue.Input2});`
                                }
                                else {
                                    var scriptValueGene = scriptValue.ReturnsValue + `=actions.${scriptValue.ActionList}(${scriptValue.PomObject},"${scriptValue.Input2}",${scriptValue.Input3});`
                                }
                            }
        ...
        function variableHandling(scriptValue, data, callback) {
            ...
            if (result.length != 0) {
                ...
                if (scriptValue.Input3 !== undefined) {
                    if (scriptValue.ReturnsValue === undefined) {
                        var scriptValueGene = `actions.${scriptValue.ActionList}("${scriptValue.Input2}","${scriptValue.Input3}");`;
                    }
                    else {
                        var scriptValueGene = scriptValue.ReturnsValue + `=actions.${scriptValue.ActionList}("${scriptValue.Input2}",${scriptValue.Input3});`;
                    }
                }
                else {
                    if (scriptValue.ReturnsValue === undefined) {
                        ...
                        var scriptValueGene = scriptValue.ReturnsValue + `=actions.${scriptValue.ActionList}(${scriptValue.Input2});`;
                    }
