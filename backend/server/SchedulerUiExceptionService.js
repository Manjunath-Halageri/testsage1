...
function handlingUiException(data,callback) 
    {  
        ...
        var findPomClassName = (data) => new Promise ((resolve,reject)=>{
            var navigateToPom = `../uploads/opal/${data.projectName}/MainProject/suites/Scheduler/schedulerExceptionHandler/${data.createdCopySuite}/src/test/java/${data.moduleId}/${data.featureId}/${data.scriptId}.java`;
     ...
     if(pageName008.includes(",")){
        ...
        var findAndChangePom = (pomDoc,pageName009) => new Promise((reslove,reject)=>{
            var domPath = `../uploads/opal/${data.projectName}/MainProject/suites/Scheduler/schedulerExceptionHandler/${data.createdCopySuite}/src/main/java/dom/${data.scriptId}.html`;
            ...
            fs.readFile(completeDomPath, 'utf8', function(err, data) {  
                ...
                pomDoc.objectName.forEach((element,index )=> {
                    ...
                    console.log(element.objectName === pageName009)
                ...
                pomDoc.objectName.forEach(element => {
                    ...
                    if(el.locators === 'id'){
                        ...
                        db.reports.update({"Run":runNo},{$set : {"exceptionObjectData" : ExceptionObjectData}},function(err,doc){
                           ...
                           // console.log(doc);
                            var pomPath = `../uploads/opal/${data.projectName}/MainProject/suites/Scheduler/schedulerExceptionHandler/${data.createdCopySuite}/src/main/java/pom/${objectPageName}.java`;
                            var completePomPath =  path.join(__dirname, pomPath)
                            return updatePom(completePomPath,message[0],el.value)
                        })
                    }
