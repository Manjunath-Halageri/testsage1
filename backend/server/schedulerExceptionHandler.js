const exec = require('child_process');
    ...
    var firstTime = 0;
    const handlingUiExp = require('./SchedulerUiExceptionService.js');
    ...
    const dataBase = require('../serverConfigs/db').database;
     ...
     const dbServer = require('./services/db');
    // var fs = require('fs');
    ...
    // async function exceptionStatusCall(req, res) {
        var  exceptionStatusCall= (req,res) => new Promise((resolve, reject) => {
      console.log('req.query.reportNum')
      console.log(req.reportNum)
      console.log('req.query.reportNum')
...
//   async function exceptionHandlingCall(req, res) {
    var exceptionHandlingCall = (req,res) => new Promise((resolve, reject) => {
      console.log('Exception handling call started')
      console.log(req)
      var runDetails = req[0];
      console.log('Exception handling call started')
  ...
  function fetchOnlyFailed(scrtiptData, data, res) {
      ...
      if (failedDetails.length != 0) {
          createExceptionObject(failedDetails, data.projectname, data.projectId, data.exceptionStatusId,data.orgId, res)
      } else {
  ...
  var writeStackTrace = (stack, script) => new Promise((resolve, reject) => {
      ...
      fs.writeFile("./uploads/opal/" + script.projectName + "/MainProject/suites/Scheduler/" + script.suiteName + "/" + script.scriptId + ".exception.txt", fullStackTrace, function (err) {
          if (err) {
  ...
  function readScript(scripts, res) {
      ...
      scripts.forEach(function (e, eindex, earray) {
          lr = new LineReader("./uploads/opal/" + e.projectName + "/MainProject/suites/Scheduler/" + e.suiteName + "/" + e.scriptId + ".exception.txt");
  ...
  function removeUnfailedScript(scriptArray) {
    ...
    var sourceSuite = `../uploads/opal/${scriptArray[0].projectName}/MainProject/suites/Scheduler/schedulerExceptionHandler/${scriptArray[0].createdCopySuite}/src/test/java/${scriptArray[0].moduleId}/${scriptArray[0].featureId}/`
  ...
  function changeScript(scripts, callback) {
      ...
      scripts.forEach(function (script, scriptIndex, scriptLength) {
          if (script.changeAt == 'Script') {
              ...
              var path12 = '../uploads/opal/' + script.projectName + "/MainProject/suites/Scheduler/schedulerExceptionHandler" + "/" + script.createdCopySuite + "/src/test/java/" + script.moduleId + "/" + script.featureId + "/" + script.scriptId + ".java";
  ...
  function increaseImplicit(scripts, callback) {
      ...
      scripts.forEach(function (e, eindex, earray) {
          if (e.changeAt == 'Config') {
              ...
              filePath = `./uploads/opal/${e.projectname}/MainProject/suites/Scheduler/schedulerExceptionHandler/${e.createdCopySuite}/src/test/java/${e.moduleId}/${e.featureId}/${e.scriptId}Config.json`;
  ...
  function changeConfig(scriptData, machineDetails) {
      ...
      scriptData.forEach(function (s, sindex, sarray) {
          ...
          var config = `../uploads/opal/${s.projectName}/MainProject/suites/Scheduler/schedulerExceptionHandler/${s.createdCopySuite}/src/test/java/${s.moduleId}/${s.featureId}/${s.scriptId}Config.json`;
        var finalConfig = path.join(__dirname, config);
          ...
          configFile.set('ExecutionType.type', "SchedulerException");
          configFile.save();
          if (sindex == sarray.length - 1) {
  ...
  function duplicateReport(data) {
    ...
    var convertedJson = path.join(__dirname, "../uploads/opal/" + data[0].projectName + "/MainProject/suites/Scheduler/schedulerExceptionHandler" + "/" + data[0].createdCopySuite + "/target/surefire-reports/Report.json")
